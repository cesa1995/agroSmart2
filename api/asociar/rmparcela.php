...
if(isset($data->jwt)){
    ...
    if($token && $validToken->nivel==0){
        ...
        if(isset($data->id) || isset($data->idparcela) || isset($data->idfinca)){
            $asociar->id=$data->id;
            $asociar->idparcela=$data->idparcela;
            $asociar->idfinca=$idfinca;

            if($asociar->deleteparcela()){
                http_response_code(200);
                echo json_encode(array("message"=>"Parcela eliminado"));
            }else{
                http_response_code(503);
                echo json_encode(array("message"=>"Parcela no eliminado"));
            }
        }else{
